...
class JournalListViewController: UITableViewController {
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if segue.identifier == "SegueListToDetail" {
      ...
      let childContext = NSManagedObjectContext(concurrencyType: .mainQueueConcurrencyType)
      childContext.parent = coreDataStack.mainContext
      let new = JournalEntry(context: childContext)
      let newChildEntry = childContext.object(with: new.objectID) as? JournalEntry
      

      detailViewController.journalEntry = newChildEntry
      detailViewController.context = childContext
      detailViewController.delegate = self
    }
  }
